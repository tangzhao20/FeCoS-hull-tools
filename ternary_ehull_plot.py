#!/usr/bin/env python3
"""
ternary_ehull_plot.py  — Gibbs network with ternary tie-lines + grid (no pymatgen)

What it does
------------
- Reads a CSV like: index,F,E0,mag,vol,nFe,nCo,nS,Eform,Ehull,Js
  (column names may also be E_form / E_hull; both are accepted).
- Keeps the lowest-Ehull entry per *reduced* composition (divide by gcd).
- Colors non-stable points by Ehull (0 → max) using a rainbow colormap.
- Marks stable points (Ehull <= zero_tol) as larger black dots with white edge.
...
- Builds the *lower* convex hull in (x=fFe, y=fCo, z=Eform) from ALL stable vertices
  (Fe/Co/S + every stable binary/ternary) and draws *all* tie-lines (including
  binary↔binary across the interior). No PhaseDiagram dependency.

Usage
-----
  python ternary_ehull_plot.py --in processed_data.csv --out ternary_Ehull.png
  python ternary_ehull_plot.py --in processed_data.csv --max-ehull 0.5 --zero-tol 1e-6 --facet-tol 1e-4 --dpi 200
"""
...
def main():
    ...
    # Accept both styles of column names
    ...
    if col_eh is None or col_ef is None:
        raise ValueError("CSV must contain Ehull/E_hull and Eform/E_form along with nFe,nCo,nS.")
    ...
    #ax.set_title("Fe–Co–S Ternary: stable (black), full Gibbs tie-lines, E_hull (rainbow)", fontsize=11)
    plt.tight_layout()
